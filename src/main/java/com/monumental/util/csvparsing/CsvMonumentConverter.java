...
public class CsvMonumentConverter {
            ...
            try {
                for (int i = 0; i < values.size(); i++) {
                    ...
                    switch (field) {
                        ...
                        case "imageAltText":
                            if (zipFile != null) {
                                result.getImageAltTexts().addAll(parseCsvArray(value));
                            } else {
    ...
    public static CreateMonumentSuggestion parseCsvMonumentConverterResult(CsvMonumentConverterResult result, Gson gson) {
        ...
        if (result.getImageAltTexts() != null && result.getImageAltTexts().size() > 0) {
            suggestion.setImageAltTextsJson(gson.toJson(result.getImageAltTexts()));
        }
