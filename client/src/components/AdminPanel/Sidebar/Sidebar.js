    ...
    get links() {
        ...
        const manageSuggestionsExtraDisplay = (<>
            <OverlayTrigger placement="top"
                            overlay={props => (
                                <Tooltip {...props}>There are currently {pendingSuggestionCount} pending
                                    Suggestions</Tooltip>
                            )}>
        ...
        return [
            ...
            {name: 'Bulk Edit', icon: 'account_balance', route: '/panel/bulk-edit', roles: Role.RESEARCHER_OR_ABOVE},
            {
                ...
                icon: 'description',
                route: '/panel/manage/suggestions',
                roles: Role.RESEARCHER_OR_ABOVE,
                extraDisplay: manageSuggestionsExtraDisplay
            },
            {
                name: 'Manage Monuments',
                icon: 'account_balance',
                route: '/panel/manage/monuments',
                roles: Role.RESEARCHER_OR_ABOVE
            },
        ...
        return (
            <div className="sidebar">
                {this.links.filter(link => (!link.roles || link.roles.includes(user.role)) && (!link.role || link.role === user.role)).map(link => (
                    <NavLink to={link.route} exact={link.exact} className="nav-link d-flex align-items-center"
                             activeClassName="active" key={link.name}>
