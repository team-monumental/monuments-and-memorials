...
export function bulkUpdateMonuments(form) {
    return async dispatch => {
        dispatch(pending(actions.update));

        try {
            // Create the BulkUpdateMonumentSuggestion
            let suggestResult = await (await fetch(actions.update.suggestUri, {
                method: 'post',
                body: buildFormData(form)
            })).json();
            ...
            await new Promise(resolve => {
                interval = window.setInterval(async () => {
                    suggestResult = await (await fetch(`${actions.update.suggestUri}/progress/${suggestJobId}`)).json();
                    // Divide the suggestion progress by 2 so it ranges from 0.0 to 0.50
                    dispatch(pending(actions.update, (suggestResult.progress / 2)));
            ...
            suggestResult = await (await fetch(`${actions.update.suggestUri}/result/${suggestJobId}`)).json();

            // Approve the BulkUpdateMonumentSuggestion
            let approveResult = await put(`${actions.update.approveUri}/${suggestResult.id}/approve`);

            const approveJobId = approveResult.id;

            await new Promise(resolve => {
                interval = window.setInterval(async () => {
                    approveResult = await (await fetch(`${actions.update.approveUri}/approve/progress/${approveJobId}`)).json();
                    // Divide the approve progress by 2 then add 0.50 so it ranges from 0.50 to 1.00
                    dispatch(pending(actions.update, ((approveResult.progress / 2) + 0.50)));
            ...
            approveResult = await (await fetch(`${actions.update.approveUri}/approve/result/${approveJobId}`)).json();
            dispatch(success(actions.update, approveResult));
        } catch (err) {
